...
const Index = () => {
  ...
  const searchHandler = async (
    e?: FormEvent | MouseEvent<HTMLLIElement>,
    value?: string
  ) => {
    (e as FormEvent).preventDefault();
    setSearchResults(
      await getData(value || (searchRef.current?.value as string))
    );
  };
  ...
  return (
    <div className="dark:bg-black min-h-screen bg-white flex flex-col items-center">
      <div className="w-11/12 sm:w-4/5 md:w-3/4 xl:w-1/2 pt-8 pb-16 flex flex-col gap-y-8">
        <TopHeader />
        <SearchBar searchHandler={searchHandler} search={searchRef} />
        {searchResults ? (
          searchResults.info === "found" ? (
            <MainContent
              search={searchHandler}
              searchContents={searchResults && searchResults.data}
            />
